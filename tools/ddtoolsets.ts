...
export type ToolsetAction = 'list' | 'enable' | 'disable' | 'reset' | 'configure';
export type DdtoolsetsParams = { action?: ToolsetAction; toolsets?: string; force?: boolean };
...
export const runDdtoolsets = async (
  ...
  return {
    content: [{ type: 'text', text: lines(label, '', 'The next Datadog tool call will use the updated toolsets.') }],
    details: { state: 'updated', toolsets: next },
  };
};

export const createDdtoolsets = (deps: ToolDeps) =>
  defineTool({
    name: 'ddtoolsets',
    ...
    async execute(...args): Promise<DdtoolsetsResult> {
      ...
      return runDdtoolsets(deps, params, ctx);
    },
  });
