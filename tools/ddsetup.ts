...
export type DdsetupParams = { site?: string; scope?: ConfigScope };
...
type DdsetupOptions = { hasAgentFollowUpNote?: boolean; hasImmediateAuthentication?: boolean };

export const runDdsetup = async (
  { mcp, urls, mcpFile, mcpEnabledToolsets, cwd, globalDir }: ToolDeps,
  params: DdsetupParams,
  ctx: ExtensionContext,
  options: DdsetupOptions = {},
): Promise<DdsetupResult> => {
      ...
      content: [
        {
          ...
          text: lines(
            ...
            'To change the domain or troubleshoot, use ddconfig.',
          ),
        },
      ],
  ...
  const authNote =
    mcp.authMode === 'oauth'
      ? options.hasImmediateAuthentication
        ? 'Pi will open your browser now to sign in to Datadog. Sign-in is shared across all projects for this domain.'
  ...
  const agentFollowUpNote =
    options.hasAgentFollowUpNote &&
...
export const createDdsetup = (deps: ToolDeps) =>
  defineTool({
    name: 'ddsetup',
    label: 'Datadog Setup',
    description: lines(
    ...
    async execute(...args): Promise<DdsetupResult> {
      ...
      return runDdsetup(deps, params, ctx, { hasAgentFollowUpNote: true });
    },
  });
