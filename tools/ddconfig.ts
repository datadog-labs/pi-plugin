...
export type DdconfigAction = 'status' | 'change-site' | 'troubleshoot';
export type DdconfigParams = { action?: DdconfigAction; site?: string };
...
export const runDdconfig = async (
  { mcp, urls, mcpFile, cwd, globalDir }: ToolDeps,
  params: DdconfigParams,
  ctx: ExtensionContext,
): Promise<DdconfigResult> => {
...
export const createDdconfig = (deps: ToolDeps) =>
  defineTool({
    name: 'ddconfig',
    ...
    async execute(...args): Promise<DdconfigResult> {
      ...
      return runDdconfig(deps, params, ctx);
    },
  });
